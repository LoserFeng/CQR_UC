...
class Decoder:
    ...
    def decode_control_frame(self,control_frame):
        ...
        type_length=8
        status_code_length=8
        total_frames_length=16  #对于 Status_code =1 传的是完整的数据大小   对于Status_code=2 传的是每个数据帧的大小
        lack_num_length=128
        variable_length=0
        ...
        # 确保payload至少有20字节
        # if len(control_frame) !=20:  #如果返回的数据长度不是20，那肯定不是通讯帧
        ...
        status_code = bs.read('uint:8')    # 2位
        total_frames = bs.read('uint:16')      # 16位
    ...
    def reassemble_data_frames(self,decoded_frame_dict,total_frames_expected=0):
        """
        重组多个数据帧，恢复原始数据。
...
class Encoder:  #Encoder 已经完成
    ...
    def generate_control_qrcode(self,status_code,total_frames,decoded_frame_dict):
        ...
        control_frame=self.generate_control_frame(frame_type,qrcode_type,status_code,total_frames,lack_num)
    ...
    def generate_data_cqrcodes(self,data, error_correct_level:str=None,qrcode_type:int=None,use_compress=None):
        ...
        for seq_num,data_bytes_split in enumerate(data_bytes_splits):
            ...
            data_frame=self.generate_data_frame(self._data_frame_type,self._qrcode_type,self._use_compress,seq_num,len(data_bytes_split),data_bytes_split)
    ...
    def generate_control_frame(self,frame_type,qrcode_type,status_code,total_frames,lack_num):  #status_code 0表示准备就绪 1表示继续传输 2表示结束
        res=0
        # status_code=2  #2位 ？
        frame_type_length=4
        qrcode_type_length=4
        status_code_length=8
        total_frames_length=16  #用帧的数量表示
        lack_num_length=128
        ...
        if not (0 <= status_code < (1 << status_code_length)):
            raise ValueError("status_code 超出范围 (0-127)")
        if not (0 <= total_frames < (1 << total_frames_length)):
            raise ValueError("total_frames 超出范围 (0-65535)")
        ...
        bs.append(f'0b{status_code:08b}')     # 8 位
        bs.append(f'0b{total_frames:016b}')       # 16 位
    ...
    def generate_data_frame(self,frame_type,qrcode_type,use_compress,seq_num,datasize,variable_data):
        frame_type_length=4
        qrcode_type_length=4
        ...
        bs.append(f'0b{seq_num:07b}')        # 7 位
