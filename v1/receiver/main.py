...
import sys
import time
from utils import get_last_seq_num,log_experiment_result
        ...
        while(loop_flag):
            ...
            if success:
                ...
                if status==0:
                    ...
                    if(qrcode_data!=None):
                        ...
                        if(control_frame_dict!=None):
                            if(control_frame_dict['status_code']==0):
                                ...
                                control_frame_img=encoder.generate_control_qrcode(status_code=status,send_rounds=send_rounds,total_frames=total_frames,decoded_frame_dict=decoded_frame_dict)
                    ...
                    if(qrcode_data!=None):
                        try:
                            control_frame=decoder.decode_control_frame(qrcode_data) # 可能会因为读取到了彩色二维码而导致出现问题
                        except Exception as e:
                            print(e)
                            print(f'error_data:{qrcode_data}')
                            cv2.imwrite('./exception/expection.jpg',img0)
                            exit(0)
