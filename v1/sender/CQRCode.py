...
class Decoder:
    def __init__(self,test=False) -> None:
        ...
        self._control_frame_type=8   #0b 1000 #直接拼接，不要编码
        self._data_frame_type=9   # 0b 1001
        pass
    ...
    def decode_control_frame(self,control_frame):
        ...
        # if(len(control_frame)!=17):
        #     print(f'control_frame: not len 17 {len(control_frame)}')
        #     return None    
        # first_char_bin = bin(ord(control_frame[0]))[2:].zfill(8)  # 获取第一个字符的 ASCII 二进制格式

        # # 检查前4位是否为 "1001"
        # if first_char_bin[:4] == "1001":
        #     print("前4位是1001")
        # else:
        #     print("前4位不是1001")
        ...
        send_rounds = bs.read('uint:4')
        use_compress=bs.read('uint:1')
        total_frames = bs.read('uint:15')      # 16位
        lack_num = bs.read('uint:128')     # 128位
...
class Encoder:  #Encoder 已经完成
    ...
    def __init__(self,version:int,qrcode_type:int,error_correct_level:str='L',use_compress=False):
        ...
        self._control_frame_type=8   #0b 1001
        self._data_frame_type=9   # 0b 1002
    ...
    def generate_control_qrcode(self,status_code,send_rounds=0,total_frames=0,decoded_frame_dict=None):
        ...
        if(decoded_frame_dict!=None):
            lack_num=self.get_lack_num(total_frames,decoded_frame_dict)
        control_frame=self.generate_control_frame(frame_type,qrcode_type,status_code,send_rounds,use_compress,total_frames,lack_num)
    ...
    def generate_control_frame(self,frame_type,qrcode_type,status_code=2,send_rounds=0,use_compress=False,total_frames=0,lack_num=0):  #status_code 0表示准备就绪 1表示继续传输 2表示结束
        res=0
        # status_code=2  #2位 ？
        frame_type_length=4
        qrcode_type_length=4
        status_code_length=4
        send_rounds_length=4
        use_compress_length=1
        total_frames_length=15  #用帧的数量表示
        lack_num_length=128
        ...
        if not (0 <= lack_num < (1 << lack_num_length)):
            raise ValueError("lack_num 超出范围 (0-2^128-1)")
        ...
        bs.append(f'0b{send_rounds:04b}')     # 8 位
        bs.append(f'0b{use_compress:01b}')       # 16 位
        bs.append(f'0b{total_frames:015b}')       # 16 位
        ...
        print(f'control_frame:{control_frame},len:{len(control_frame)}')
