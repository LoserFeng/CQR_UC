...
import base64
import zlib
from utils import add_white_frame
...
class Decoder:
    def __init__(self,test=False) -> None:
        ...
        self._control_frame_type=2   #0b 10
        self._data_frame_type=3   # 0b 11
        pass
    ...
    def CQRCode_decode(self,img,qrcode_type:int,test:bool=False,super:bool=False):  # super表示是否需要自调整颜色阀域
        ...
        if(color_list[3]==1 and self.test):  #
    ...
    def decode_lack_num_list(self,lack_num_list,lack_num_len=128):  #计算编号

        lack_frame_idx_list=[]
        for idx,lack_num in enumerate(lack_num_list):
            end_flag=bool((1<<(lack_num_len-1))&lack_num)  #第一位用于判断是否结束
            if(end_flag):
                lack_num^=(1<<(lack_num_len-1))  #将第一位置为0

            
            i=0
            while(lack_num!=0):
                if(lack_num&1):
                    lack_frame_idx_list.append(i+idx*(lack_num_len-1))
                lack_num>>=1
                i+=1



        return lack_frame_idx_list
    ...
    def decode_control_frame(self,control_frame):
        ...
        if(control_frame[0]!=str(self._control_frame_type)):
            return None
        control_frame=control_frame[1:] #去掉 control_frame_type
        ...
        status_code = bs.read('uint:3')    # 2位
        parity_code = bs.read('uint:1')    # 1位
        send_rounds = bs.read('uint:6')
        use_compress=bs.read('uint:2')  #压缩等级
        total_frames = bs.read('uint:14')      # 16位
        lack_num = bs.read('uint:128')     # 128位  #如果第一位为1，表示还没有传完
    ...
    def decode_data_frame(self,data_frame):
        ...
        :return: dict，包含各个字段
        """
        if(data_frame[0]==str(self._control_frame_type)):
            return None
    ...
    def reassemble_data_frames(self,decoded_frame_dict,use_compress:int=0,total_frames_expected=0):
        """
        重组多个数据帧，恢复原始数据。
...
class Encoder:  #Encoder 已经完成
    ...
    def __init__(self,version:int,qrcode_type:int,error_correct_level:str='L',use_compress:int=0):
        ...
        self._control_frame_type=2   #0b 10
        self._data_frame_type=3   # 0b 11
        self._use_compress=use_compress  #压缩等级
    ...
    def generate_qrcode(self,data, filename=None,version:int=None,error_correct_level:str=None,back_color='white',border=1):  #返回  h w 3 类型的 numpy
        ...
        # numpy_image=add_white_frame(numpy_image)
    ...
    def get_lack_num_list(self,total_frames,decoded_frame_dict,lack_num_len=128):  #根据缺少的图片的序号计算一个int类型的不同序号的和
        res=0
        lack_num_list=[]
        idx=0
        lack_num=0
        for idx in range(total_frames):
            # if((total_frames-1)//(lack_num_len-1)==idx//(lack_num_len-1)):  #如果是最后一个控制帧的话  
            #     lack_num|=(1<<(lack_num_len-1))

            if(idx%(lack_num_len-1)==0 and idx!=0):
                lack_num_list.append(lack_num)
                lack_num=0
            i=idx%(lack_num_len-1)
            if idx not in decoded_frame_dict:
                lack_num|=(1<<i)

        
        if(lack_num!=0 or (lack_num==0 and len(lack_num_list)==0)):  #如果没有lack_num，那么强行加一个
            lack_num_list.append(lack_num)

        lack_num_list[-1]=lack_num_list[-1]|(1<<lack_num_len-1)  #最后一个控制帧的最高位为1
    ...
    def generate_control_qrcodes(self,status_code,parity_code,send_rounds=0,total_frames=0,decoded_frame_dict=None):
        frame_type=self._control_frame_type
        qrcode_type=self._qrcode_type
        use_compress=self._use_compress
        lack_num_list=[0]  #默认只有一帧
        if(decoded_frame_dict!=None):
            lack_num_list=self.get_lack_num_list(total_frames,decoded_frame_dict)
        control_frames=[]
        for lack_num in lack_num_list:
            control_frame=self.generate_control_frame(frame_type,qrcode_type,status_code,parity_code,send_rounds,use_compress,total_frames,lack_num)
            control_frames.append(control_frame)
            parity_code^=1  #每一帧之后都要取反
    ...
    def generate_data_cqrcodes(self,data):
        ...
        for seq_num,data_bytes_split in enumerate(data_bytes_splits):
            ...
            data_frame=self.generate_data_frame(self._data_frame_type,seq_num,len(data_bytes_split),data_bytes_split)
    ...
    def split_data_bytes(self,data_bytes, max_part_size,use_compress=0):
        ...
        if(use_compress):  #先压缩，再分组
            data_bytes=zlib.compress(data_bytes,level=use_compress)
    ...
    def generate_control_frame(self,frame_type,qrcode_type,status_code=2,parity_code=1,send_rounds=0,use_compress=False,total_frames=0,lack_num=0):  #status_code 0表示准备就绪 1表示继续传输 2表示结束
        res=0
        # status_code=2  #2位 ？
        frame_type_length=2
        qrcode_type_length=4
        status_code_length=3
        parity_code_length=1
        send_rounds_length=6

        use_compress_length=2
        total_frames_length=14  #用帧的数量表示
        lack_num_length=128
        ...
        if not (0 <= status_code < (1 << status_code_length)):
            raise ValueError(f"status_code 超出范围 (0-{(1 << status_code_length) - 1})")
        if not(0 <= parity_code < (1 << parity_code_length)):
            raise ValueError(f"parity_code 超出范围 (0-{(1 << parity_code_length) - 1})")

        if not (0 <= send_rounds < (1 << send_rounds_length)):
            raise ValueError(f"send_rounds 超出范围 (0-{(1 << send_rounds_length) - 1}")
        ...
        if not (0 <= total_frames < (1 << total_frames_length)):
            raise ValueError(f"total_frames 超出范围 (0-{(1 << total_frames_length) - 1})")
        if not (0 <= lack_num < (1 << lack_num_length)):
            raise ValueError(f"lack_num 超出范围 (0-{(1 << lack_num_length) - 1})")


        bs = BitStream()
        bs.append(f'0b{frame_type:02b}')      # 4 位
        bs.append(f'0b{qrcode_type:04b}')      # 4 位
        bs.append(f'0b{status_code:03b}')      # 3 位
        bs.append(f'0b{parity_code:01b}')                    #奇偶位  Parity Bit
        bs.append(f'0b{send_rounds:06b}')     # 4 位
        bs.append(f'0b{use_compress:02b}')       # 1 位
        bs.append(f'0b{total_frames:014b}')       # 15 位
        ...
        control_frame=str(self._control_frame_type).encode('utf-8')+control_frame  #两者做一个拼接
    ...
    def generate_data_frame(self,frame_type,seq_num,datasize,variable_data):
        frame_type_length=2
        # qrcode_type_length=4
        ...
        # 创建 BitStream 对象
        bs = BitStream()
        bs.append(f'0b{frame_type:02b}')      # 2 位  注意这里0必须要有
        # bs.append(f'0b{qrcode_type:04b}')      # 4 位
        # bs.append(f'0b{use_compress:01b}')        # 1 位
        
        bs.append(f'0b{seq_num:014b}')        # 14 位
        bs.append(f'0b{datasize:016b}')      # 20 位
